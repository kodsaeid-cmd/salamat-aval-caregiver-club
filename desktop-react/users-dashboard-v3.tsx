...
function KpiGauge({kind,label,numerator,denominator,percent}:{kind:"dispatch"|"contract";label:string;numerator:number;denominator:number;percent:number}){
 ...
 return <article className={`rdb3-kpi-gauge ${kind}`}><div className="rdb3-kpi-copy"><span><Icon size={18}/></span><div><small>{label}</small><strong>{fa(numerator)} <em>از {fa(denominator)}</em></strong><p>{kind==="dispatch"?"پرونده دارای متقاضی در اعزام نسبت به کل آگهی‌ها":"مراقب در قرارداد نسبت به کل قراردادها"}</p></div></div><div className="rdb3-gauge" style={{"--p":`${safe}%`} as React.CSSProperties}><div><strong>{Number(percent||0).toLocaleString("fa-IR",{maximumFractionDigits:1})}٪</strong><small>KPI</small></div></div></article>;
}
