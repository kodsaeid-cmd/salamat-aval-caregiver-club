...
export function SmsControlCenterPage({notify}:{notify:Notify}){
 ...
 const s=data?.summary||{},r=readiness||{},queueCount=Number(s.pending||0)+Number(s.retrying||0)+Number(s.processing||0)+Number(s.automaticQueuePending||0);
 return <div className="smsc-page da-stack">
  ...
  <div className="smsc-metrics">
   ...
   <Card><small>کل صف و Retry</small><strong>{fa(queueCount)}</strong><span>{fa(s.automaticQueuePending||0)} مورد از اتوماسیون‌های قبلی</span></Card>
  ...
  <Card className="smsc-table-card">
   <div className="smsc-toolbar"><div><h3>صف‌های اتوماتیک موجود</h3><small>فعال‌سازی حساب، یادآوری بانک آگهی و تغییر وضعیت درخواست؛ این بخش نشان می‌دهد پیام قبل از رسیدن به SMS.ir کجا متوقف شده است.</small></div></div>
   {(data?.automaticQueues||[]).length?<div className="smsc-table-wrap"><table className="smsc-table"><thead><tr><th>زمان</th><th>اتوماسیون</th><th>گیرنده</th><th>وضعیت صف</th><th>تلاش بعدی</th><th>شناسه / خطا</th></tr></thead><tbody>{(data.automaticQueues||[]).map((row:any)=><tr key={`${row.queueType}:${row.id}`}><td>{dateFa(row.createdAt)}</td><td><strong>{queueTypeFa[row.queueType]||row.queueType}</strong><small>{row.contextLabel||"—"}</small></td><td><strong>{row.recipientName||"—"}</strong><small dir="ltr">{row.recipientMobile||"—"}</small></td><td><span className={`smsc-outbox ${String(row.status||"").toLowerCase()}`}>{outboxFa[row.status]||row.status}</span><small>{fa(row.attemptCount)} تلاش</small></td><td>{row.nextAttemptAt?dateFa(row.nextAttemptAt):row.sentAt?`ارسال: ${dateFa(row.sentAt)}`:"—"}</td><td><strong className="smsc-code">{row.providerMessageId||"—"}</strong>{row.lastError&&<small className="smsc-error">{row.lastError}</small>}</td></tr>)}</tbody></table></div>:<Empty title="صف اتوماتیک قابل نمایشی وجود ندارد" description="اگر migrationهای پیامک فعال باشند، رویدادهای اتوماتیک اینجا دیده می‌شوند."/>}
  </Card>
