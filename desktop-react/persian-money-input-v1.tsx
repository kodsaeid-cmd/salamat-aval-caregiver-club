...
import "./persian-money-input-v1.css";
...
const englishGrouped=(raw:string)=>raw?Number(raw).toLocaleString("en-US"):"";
export function PersianRialInput({name="caregiverSalaryRial",required=true}:{name?:string;required?:boolean}){const [raw,setRaw]=useState("");const amount=Number(raw||0),toman=Math.floor(amount/10);return <div className="ja-money-field"><input type="hidden" name={name} value={raw}/><input className="ja-money-input" type="text" inputMode="numeric" autoComplete="off" dir="ltr" value={englishGrouped(raw)} onChange={e=>setRaw(normalizeNumericText(e.target.value))} placeholder="150,000,000" aria-label="حقوق مراقب به ریال" required={required}/><div className="ja-money-words">{raw?`${numberToPersianWords(toman)} تومان`:"مبلغ به حروف تومان همزمان اینجا نمایش داده می‌شود"}</div></div>}
export function EnglishNumericInput({name,required=true,min=0,max,placeholder=""}:{name:string;required?:boolean;min?:number;max?:number;placeholder?:string}){const [raw,setRaw]=useState("");return <><input type="hidden" name={name} value={raw}/><input type="text" inputMode="numeric" autoComplete="off" dir="ltr" value={englishGrouped(raw)} placeholder={placeholder} required={required} onChange={e=>{let next=normalizeNumericText(e.target.value);if(next&&Number(next)<min)next=String(min);if(next&&max!=null&&Number(next)>max)next=String(max);setRaw(next)}}/></>}
