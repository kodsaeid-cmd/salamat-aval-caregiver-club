...
let activeUsersStatus="",activeUsersRole="",lastUsersListUrl="";
...
function selectedRoleFromFilter(root:HTMLElement|null){if(!root)return"";const labels=Array.from(root.querySelectorAll("label")),roleLabel=labels.find(label=>String(label.querySelector("span")?.textContent||"").includes("سمت سازمانی"));return String(roleLabel?.querySelector<HTMLSelectElement>("select")?.value||"").trim().toUpperCase()}
function ensureUsersInterceptor(){
 ...
 window.fetch=(async(input:RequestInfo|URL,init?:RequestInit)=>{
  ...
  if(url.pathname==="/api/users"){if(activeUsersRole)url.searchParams.set("role",activeUsersRole);else url.searchParams.delete("role")}
...
export function UsersAccessPageV4({access,notify}:{access:any;notify:Notify}){
 ...
 useEffect(()=>{const filterButton=commandTarget?.querySelectorAll<HTMLButtonElement>("button.uav3-toolbtn")?.[1];if(!filterButton)return;filterButton.classList.toggle("active",Boolean(appliedStatus||activeUsersRole));return()=>filterButton.classList.remove("active")},[commandTarget,appliedStatus]);
