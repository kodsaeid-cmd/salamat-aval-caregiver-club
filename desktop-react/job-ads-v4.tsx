...
let activePage=1,lastListSignature="",requestSerial=0;
...
function ensureInterceptor(){
 ...
 window.fetch=(async(input:RequestInfo|URL,init?:RequestInit)=>{
  ...
  const signature=filterSignature(url);if(lastListSignature&&lastListSignature!==signature)activePage=1;lastListSignature=signature;
...
export function JobAdsPage({notify}:{notify:Notify}){
 ...
 useEffect(()=>{const sync=(event:Event)=>setPagination((event as CustomEvent<Pagination>).detail||DEFAULT_PAGINATION);window.addEventListener(PAGINATION_EVENT,sync);return()=>{window.removeEventListener(PAGINATION_EVENT,sync);activeFilters=DEFAULT_FILTERS;activePage=1;lastListSignature="";requestSerial++}},[]);
 useEffect(()=>{api<any>("/api/staff/job-ads/consultants?q=").then(p=>setConsultants(p.data?.consultants||[])).catch(()=>setConsultants([]))},[]);
 const reloadFromFirstPage=()=>{activePage=1;lastListSignature="";requestSerial++;setPagination(DEFAULT_PAGINATION);submitVisibleJobList()};
 ...
 const goPage=(next:number)=>{if(next<1||next>pagination.totalPages||next===pagination.page)return;activePage=next;requestSerial++;setPagination(current=>({...current,page:next,hasPrevious:next>1,hasNext:next<current.totalPages}));submitVisibleJobList();window.scrollTo({top:0,behavior:"smooth"})};
