...
export function JalaliDateFilter({value,onChange,placeholder="تاریخ ایجاد پروفایل",compact=false}:{value:string;onChange:(value:string)=>void;placeholder?:string;compact?:boolean}){
 ...
 const label=value&&selected?persianLabelFormatter.format(selected):placeholder;
 const choose=(date:Date)=>{onChange(key(date));setAnchor(date);setOpen(false)};
 return <div className={`jdf-root${compact?" compact":""}`}>
  <button type="button" className={`jdf-trigger ${value?"active":""}`} aria-label={placeholder} onClick={()=>setOpen(v=>!v)}><CalendarDays size={compact?15:17}/><span>{label}</span>{value&&<span className="jdf-clear" role="button" tabIndex={0} aria-label="پاک‌کردن تاریخ" onClick={e=>{e.stopPropagation();onChange("")}} onKeyDown={e=>{if(e.key==="Enter"){e.stopPropagation();onChange("")}}}><X size={compact?12:14}/></span>}</button>
