...
function installForm(form){
  ...
  people.innerHTML=`<div class="assignment-block-title"><strong>انتخاب مراقبین</strong><small>جست‌وجو و صفحه‌بندی در کل دیتابیس</small></div><form class="trp-toolbar" id="trpSearchForm"><input id="trpSearch" placeholder="جست‌وجوی نام، شماره پرونده، موبایل یا گروه خدمتی"><button class="trp-btn" type="submit">جست‌وجو</button><button class="trp-btn" type="button" id="trpSelectPage">انتخاب این صفحه</button><button class="trp-btn soft" type="button" id="trpClear">پاک‌کردن انتخاب‌ها</button></form><div class="trp-summary"><span>انتخاب‌ها هنگام رفتن به صفحات بعدی حفظ می‌شوند.</span><strong><span id="trpSelected">۰</span> نفر</strong></div><div class="trp-list" id="trpList"></div><footer class="trp-footer" id="trpFooter"></footer>`;
  $('#trpSearchForm')?.addEventListener('submit',event=>{event.preventDefault();runSearch()});
