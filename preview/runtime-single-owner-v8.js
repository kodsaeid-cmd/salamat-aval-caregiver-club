...
function activeModuleKey(){return buttonKey($('#sidebarNav .active'))||String(window.SalamatStaffModuleRouter?.access?.activeKey||'')}
function evaluationRouteVisible(){return activeModuleKey()==='staff.evaluations'||String($('#pageTitle')?.textContent||'').includes('ارزیابی')}
...
function loadMobileEvaluationDrilldown(force=false){
  if(!window.matchMedia?.('(max-width:760px)').matches)return false;
  // Evaluation UI must never participate in the signed-out/login lifecycle.
  // Load it only inside the authenticated panel and only for the evaluation route.
  if(!appVisible())return false;
  if(!force&&!evaluationRouteVisible())return false;
