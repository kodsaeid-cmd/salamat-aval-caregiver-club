(()=>{
'use strict';
if(window.__salamatCaregiverTemporaryUiGateV1)return;
window.__salamatCaregiverTemporaryUiGateV1=true;
...
function boot(){
  ...
  const timer=setInterval(()=>{
    attempts+=1;
    if(!observersInstalled)installObservers();
    schedule();
    if((caregiverActive()&&$('#sidebarNav')&&$('#content'))||attempts>=40)clearInterval(timer);
  },125);
  window.SalamatCaregiverTemporaryUiGate={version:VERSION,enforce:schedule,callNumber:CALL_NUMBER};
}
