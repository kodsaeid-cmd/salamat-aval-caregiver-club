...
function rowsMarkup(items){
  ...
  return items.map(item=>`<button class="cdp-row" type="button" data-cdp-id="${esc(item.id)}">${avatar(item)}<span><strong>${esc(item.fullName)}</strong><small>شماره پرونده ${esc(item.membershipCode||item.id)} • ${esc(item.mobile||'شماره ثبت نشده')}</small></span><span class="cdp-cell"><small>گروه خدمتی</small><b>${esc(item.primaryType||'—')}</b></span><span class="cdp-cell"><small>وضعیت پرونده</small><b>${esc(item.fileStatus||'—')}</b></span><span class="cdp-cell optional"><small>سطح حرفه‌ای</small><b>${esc(item.professionalLevel||'در انتظار ارزیابی')}</b></span><span class="cdp-cell optional"><small>حساب ورود</small><b>${item.hasAccount?'متصل':'بدون حساب'}</b></span><span class="cdp-badge">مشاهده و ویرایش</span></button>`).join('')
}
async function render(){
  if(!isAdmin())return;
  const current=++requestId;
  if($('.cdp-root'))$('.cdp-panel')?.classList.add('busy');
  else setPage('پرونده مراقبین','نمایش صفحه‌بندی‌شده پرونده‌های حرفه‌ای','<div class="cdp-loading">در حال دریافت ۵۰ پرونده...</div>');
  try{
    ...
    lastItems=data.items||[];const p={page:1,total:0,totalPages:1,hasPrevious:false,hasNext:false,...(data.pagination||{})};page=Number(p.page||1);
    setPage('پرونده مراقبین','نمایش صفحه‌بندی‌شده پرونده‌های حرفه‌ای',`<div class="cdp-toolbar"><div><h3>پرونده‌های حرفه‌ای مراقبین</h3><p>${fa(p.total)} پرونده؛ برای جست‌وجو Enter یا دکمه جست‌وجو را بزنید.</p></div><div class="cdp-actions"><input class="cdp-search" id="cdpSearch" value="${esc(query)}" placeholder="نام، موبایل، کد ملی یا شماره پرونده"><button class="cdp-btn primary" id="cdpSearchButton">جست‌وجو</button><button class="cdp-btn" id="cdpClearSearch">پاک‌کردن</button><button class="cdp-btn primary" id="cdpCreate">ایجاد پرونده جدید</button></div></div><article class="cdp-panel"><div class="cdp-list">${rowsMarkup(lastItems)}</div><footer class="cdp-footer"><span>نمایش ${fa(lastItems.length)} مورد از ${fa(p.total)} پرونده</span><div class="cdp-pages"><button class="cdp-btn" id="cdpPrevious" ${p.hasPrevious?'':'disabled'}>صفحه قبل</button><strong>صفحه ${fa(p.page)} از ${fa(p.totalPages)}</strong><button class="cdp-btn" id="cdpNext" ${p.hasNext?'':'disabled'}>صفحه بعد</button></div></footer></article>`);
    bind(p);
  }catch(error){if(current===requestId)setPage('پرونده مراقبین','خطا در دریافت اطلاعات',`<div class="cdp-empty">${esc(error.message||error)}</div>`)}
}
function runSearch(){query=String($('#cdpSearch')?.value||'').trim();page=1;void render()}
function bind(p){
  ...
  $('#cdpClearSearch')?.addEventListener('click',()=>{query='';page=1;void render()});
...
function installRouter(){
  const current=window.renderModule;if(typeof current!=='function'||current.__salamatCaregiverPaginationV3)return;
  ...
  wrapped.__salamatCaregiverPaginationV3=true;window.renderModule=wrapped;try{renderModule=wrapped}catch{}
}
function boot(){
  addStyles();window.addEventListener('click',capture,true);installRouter();setInterval(installRouter,1200);
  window.addEventListener('salamat-server-directory-refresh',()=>{const title=String($('#pageTitle')?.textContent||'');if(title.includes('پرونده مراقبین'))void render()});
  window.addEventListener('salamat-caregiver-profile-updated',()=>{if($('.cdp-root'))void render()});
}
