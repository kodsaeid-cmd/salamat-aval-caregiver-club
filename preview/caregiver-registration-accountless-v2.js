(()=>{
  ...
  const removeCredentialFields=form=>{
    for(const name of HIDDEN_NAMES){
      ...
      input.disabled=true;input.removeAttribute('required');
  ...
  async function submitProfile(event){
    ...
    const submit=form.querySelector('button[type="submit"],input[type="submit"]');if(submit)submit.disabled=true;show(form,'در حال تشکیل پرونده مراقب...');
    try{
    ...
    }catch(error){show(form,error?.message||'ثبت پرونده انجام نشد.',true)}finally{if(submit)submit.disabled=false}
  }
  function activate(){const form=document.getElementById(ROOT_ID);if(!(form instanceof HTMLFormElement)||form.dataset.accountlessV2==='1')return;form.dataset.accountlessV2='1';removeCredentialFields(form);form.addEventListener('submit',submitProfile,true)}
