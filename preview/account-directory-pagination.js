(()=>{
'use strict';
if(window.__salamatAccountDirectoryPaginationV3)return;
window.__salamatAccountDirectoryPaginationV3=true;
...
function addStyles(){
  if($('#accountDirectoryPaginationStylesV3'))return;
  const style=document.createElement('style');
  style.id='accountDirectoryPaginationStylesV3';
  style.textContent=`
...
function bind(p){
  ...
  $$('[data-account-id]').forEach(row=>row.addEventListener('click',()=>{
    if(row.dataset.caregiverId)return;
