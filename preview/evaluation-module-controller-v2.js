(()=>{
'use strict';
if(window.__salamatEvaluationModuleControllerV2)return;
window.__salamatEvaluationModuleControllerV2=true;

/* Retire all historical evaluation routers and runtimes. */
...
function loadRuntime(){
  if(window.SalamatEvaluationModuleV4)return Promise.resolve(window.SalamatEvaluationModuleV4);
  if(runtimePromise)return runtimePromise;
  runtimePromise=new Promise((resolve,reject)=>{
    const existing=document.querySelector('script[data-server-evaluation-runtime-v4]');
    if(existing){
      existing.addEventListener('load',()=>resolve(window.SalamatEvaluationModuleV4),{once:true});
      existing.addEventListener('error',()=>reject(new Error('بارگذاری ماژول ارزیابی انجام نشد.')),{once:true});
      return;
    }
    const script=document.createElement('script');
    script.src='./server-evaluation-runtime-v4.js?v=1.0.0';
    script.async=true;
    script.dataset.serverEvaluationRuntimeV4='true';
    script.onload=()=>window.SalamatEvaluationModuleV4?resolve(window.SalamatEvaluationModuleV4):reject(new Error('ماژول ارزیابی آماده نشد.'));
...
function close(){
  openToken+=1;
  try{window.SalamatEvaluationModuleV4?.close?.()}catch{}
}
...
function installRenderBridge(){
  ...
  if(typeof current!=='function'||current.__salamatEvaluationControllerV2)return Boolean(current?.__salamatEvaluationControllerV2);
  ...
  wrapped.__salamatEvaluationControllerV2=true;
