...
function role(){
  try{return String(window.SalamatBackend?.getCurrentUser?.()?.role||window.__salamatPanelAccessV2?.user?.role||'').toUpperCase()}catch{return ''}
}
...
async function loadData(force=false){
  ...
  state.promise=api(ENDPOINT).then(payload=>{
...
function enhanceSignup(){
  ...
  input.setAttribute('placeholder','مثلاً 482731');
  if(!input.dataset.refv2Bound){
    input.dataset.refv2Bound='1';
    input.addEventListener('input',()=>{input.value=String(input.value||'').replace(/\D/g,'').slice(0,6)});
  }
  const field=input.closest('label');
  const hint=field?.querySelector('.ref-signup-hint');
  if(hint)hint.textContent='کد معرف ۶ رقمی است. پس از ثبت‌نام، ابتدا معرف و سپس مدیر سامانه باید معرفی را تأیید کنند.';
}

function statusClass(status){
  if(['WAITING_REFERRER_CONFIRMATION','PENDING_REGISTRATION_REVIEW','WAITING_CONTRACT'].includes(status))return 'pending';
  if(['REFERRER_REJECTED','REGISTRATION_REJECTED','CONTRACT_REJECTED'].includes(status))return 'rejected';
  return '';
}
...
function walletMarkup(data){
  ...
  const progress=Math.max(0,Math.min(10,Number(summary.progressInCycle||0)));
  const pct=progress*10;
  const remaining=Number(summary.remainingToMilestone||10);
  const code=caregiver.referralCode||'—';
  return `
    ${confirmationMarkup(cases)}
...
async function renderWallet(){
  ...
  $$('#caregiverReferralRewardsV1').forEach(node=>node.classList.add('refv2-old-hidden'));
  try{const data=await loadData();if(!isWallet()||!root.isConnected)return;root.innerHTML=walletMarkup(data)}catch(error){if(root.isConnected)root.innerHTML=`<div class="refv2-error">${esc(error.message||'اطلاعات معرفی دریافت نشد.')}</div>`}
}

async function renderDashboardCode(){
  if(!isDashboard())return;
  const side=$('.cgr3-dashboard-side')||$('.cgp-hero-side');
  if(!side)return;
  try{
    const data=await loadData();if(!isDashboard()||!side.isConnected)return;
    const code=data?.caregiver?.referralCode||'—';
    const pending=Number(data?.summary?.awaitingMyConfirmation||0);
    ...
    card.innerHTML=`<small>کد معرف من</small><span class="refv2-dashboard-row"><strong>${esc(code)}</strong><button type="button" class="refv2-dashboard-copy" data-refv2-copy="${esc(code)}">کپی</button></span>${pending?`<small class="refv2-dashboard-pending">${fa(pending)} معرفی منتظر تأیید شما</small>`:''}`;
  }catch{}
}
...
async function decide(id,action){
  ...
  try{
    let note='';
    if(action==='REJECT')note=window.prompt('در صورت تمایل علت رد معرفی را بنویسید:','')||'';
    ...
    state.data=null;state.lastLoadedAt=0;
    await renderWallet();
    await renderDashboardCode();
  }catch(error){toast('ثبت تصمیم انجام نشد',error.message||'دوباره تلاش کنید.')}
...
function bindEvents(){
  ...
  document.addEventListener('click',event=>{
    const copy=event.target.closest?.('[data-refv2-copy]');
    if(copy){event.preventDefault();copyCode(copy.dataset.refv2Copy);return}
    const confirm=event.target.closest?.('[data-refv2-confirm]');
    if(confirm){event.preventDefault();decide(confirm.dataset.refv2Confirm,'CONFIRM');return}
    const reject=event.target.closest?.('[data-refv2-reject]');
    if(reject){event.preventDefault();decide(reject.dataset.refv2Reject,'REJECT')}
  });
}

function enforce(){
  state.scheduled=false;
  addStyles();enhanceSignup();
  if(!caregiverActive())return;
  renderDashboardCode();renderWallet();
  if(!isWallet())$('#caregiverReferralRewardsV2')?.remove();
}
