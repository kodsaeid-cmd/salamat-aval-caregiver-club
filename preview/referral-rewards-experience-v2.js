...
const state={data:null,promise:null,lastLoadedAt:0,timer:0,decisionBusy:false,observers:[]};
const statusLabels={WAITING_REFERRER_CONFIRMATION:'منتظر تأیید شما',REFERRER_REJECTED:'توسط شما رد شده',PENDING_REGISTRATION_REVIEW:'منتظر تأیید مدیر سامانه',WAITING_CONTRACT:'منتظر تأیید ورود به قرارداد',COMPLETED:'معرفی موفق و قراردادی',REGISTRATION_REJECTED:'مرحله ثبت‌نام رد شده',CONTRACT_REJECTED:'مرحله قرارداد رد شده'};
...
async function api(path,options={}){
  ...
  const response=await fetch(path,{credentials:'same-origin',cache:'no-store',...options,headers});const text=await response.text();let payload={};try{payload=text?JSON.parse(text):{}}catch{payload={detail:text}}
...
async function loadData(force=false){
  ...
  state.promise=api(ENDPOINT).then(payload=>{state.data=payload.data||{};state.lastLoadedAt=Date.now();return state.data}).finally(()=>{state.promise=null});return state.promise;
}
...
function confirmationMarkup(cases){
  const pending=cases.filter(item=>(item.effectiveStatus||item.status)==='WAITING_REFERRER_CONFIRMATION');if(!pending.length)return '';
  return `<section class="refv2-card"><header class="refv2-head"><div><h3>معرفی‌های منتظر تأیید من</h3><p>فقط معرفی‌هایی که واقعاً کد را در اختیارشان گذاشته‌اید تأیید کنید.</p></div><span class="refv2-badge pending">${fa(pending.length)} مورد</span></header><div class="refv2-body"><div class="refv2-list">${pending.map(item=>`<article class="refv2-item refv2-confirm"><div><strong>${esc(item.referredName||'مراقب جدید')}</strong><small>ثبت در ${dateTimeFa(item.createdAt)}<br>این فرد هنگام ثبت‌نام از کد معرف شما استفاده کرده است.</small></div><div class="refv2-actions"><button type="button" class="refv2-btn danger" data-refv2-reject="${esc(item.id)}">رد</button><button type="button" class="refv2-btn primary" data-refv2-confirm="${esc(item.id)}">تأیید معرفی</button></div></article>`).join('')}</div></div></section>`;
}
function walletSignature(data){const s=data.summary||{},cases=Array.isArray(data.cases)?data.cases:[],milestones=Array.isArray(data.milestones)?data.milestones:[];return JSON.stringify([data.caregiver?.referralCode,s.totalReferrals,s.completedReferrals,s.confirmedRewardToman,s.pendingRewardToman,s.awaitingMyConfirmation,s.progressInCycle,cases.map(x=>[x.id,x.effectiveStatus||x.status,x.updatedAt]),milestones.map(x=>[x.milestoneNumber,x.awardedAt])])}
function walletMarkup(data){
  const caregiver=data.caregiver||{},summary=data.summary||{},cases=Array.isArray(data.cases)?data.cases:[],milestones=Array.isArray(data.milestones)?data.milestones:[];const progress=Math.max(0,Math.min(10,Number(summary.progressInCycle||0))),remaining=Number(summary.remainingToMilestone||10),code=caregiver.referralCode||'—';
  return `${confirmationMarkup(cases)}<section class="refv2-card"><header class="refv2-head"><div><h3>معرفی و اعتبارات مراقب</h3><p>پاداش‌های قطعی معرفی مستقیماً در تراکنش‌های همین کیف پول ثبت می‌شوند.</p></div></header><div class="refv2-body"><div class="refv2-code"><div><small>کد معرف ۶ رقمی و یکتای من</small><strong>${esc(code)}</strong></div><button class="refv2-copy" type="button" data-refv2-copy="${esc(code)}">کپی کد</button></div></div></section><section class="refv2-kpis"><article class="refv2-kpi"><small>کل معرفی‌ها</small><strong>${fa(summary.totalReferrals||0)}</strong></article><article class="refv2-kpi"><small>معرفی منجر به قرارداد</small><strong>${fa(summary.completedReferrals||0)}</strong></article><article class="refv2-kpi"><small>اعتبار قطعی معرفی</small><strong>${money(summary.confirmedRewardToman||0)}</strong></article><article class="refv2-kpi"><small>اعتبار در انتظار</small><strong>${money(summary.pendingRewardToman||0)}</strong></article></section><section class="refv2-grid"><article class="refv2-card"><header class="refv2-head"><div><h3>تا پاداش ۵ میلیون تومانی بعدی</h3><p>هر ۱۰ معرفی موفق که قراردادشان توسط مدیر تأیید شود.</p></div></header><div class="refv2-gauge-wrap"><div class="refv2-gauge" style="--pct:${progress*10}"><div class="refv2-gauge-center"><strong>${fa(progress)} / ۱۰</strong><small>معرفی موفق این دوره</small></div></div><div class="refv2-gauge-copy"><strong>${fa(remaining)} معرفی دیگر تا اعتبار ${money(summary.nextMilestoneRewardToman||5000000)}</strong><small>هدف بعدی: ${fa(summary.nextMilestoneTarget||10)} معرفی موفق قراردادی</small></div><div class="refv2-policy"><span>ثبت‌نام تأییدشده +۲۰۰ هزار</span><span>قرارداد تأییدشده +۳۰۰ هزار</span><span>هر ۱۰ قرارداد +۵ میلیون</span></div></div></article><article class="refv2-card"><header class="refv2-head"><div><h3>سابقه پاداش‌های ۵ میلیونی</h3><p>هر دوره تکمیل‌شده در کیف پول ثبت می‌شود.</p></div></header><div class="refv2-body">${milestones.length?`<div class="refv2-list">${milestones.map(item=>`<div class="refv2-item"><div><strong>تکمیل ${fa(item.milestoneNumber)} معرفی موفق</strong><small>${dateTimeFa(item.awardedAt)}</small></div><span class="refv2-amount">+ ${money(item.rewardToman||5000000)}</span></div>`).join('')}</div>`:'<div class="refv2-empty">هنوز دوره ۱۰ معرفی موفق کامل نشده است.</div>'}</div></article></section><section class="refv2-card"><header class="refv2-head"><div><h3>وضعیت معرفی‌های من</h3><p>مسیر هر معرفی را از استفاده از کد تا قرارداد پیگیری کنید.</p></div></header><div class="refv2-body">${cases.length?`<div class="refv2-list">${cases.map(item=>{const status=item.effectiveStatus||item.status;return `<article class="refv2-item"><div><strong>${esc(item.referredName||'مراقب معرفی‌شده')}</strong><small>${dateTimeFa(item.createdAt)}<br>${esc(caseDetail(item))}</small></div><div class="refv2-side"><span class="refv2-badge ${statusClass(status)}">${esc(statusLabels[status]||status)}</span><span class="refv2-amount">قطعی: ${money(confirmedAmount(item))}</span></div></article>`}).join('')}</div>`:'<div class="refv2-empty">هنوز مراقبی با کد معرف شما ثبت‌نام نکرده است.</div>'}</div></section>`;
}

async function renderWallet(){
  if(!isWallet())return;const content=$('#content');if(!content)return;
  let root=$('#caregiverReferralRewardsV2');if(!root){root=document.createElement('section');root.id='caregiverReferralRewardsV2';root.className='refv2-root';root.innerHTML='<div class="refv2-card"><div class="refv2-loading">در حال دریافت وضعیت معرفی و اعتبارات…</div></div>';content.appendChild(root)}
  try{const data=await loadData();if(!isWallet()||!root.isConnected)return;const signature=walletSignature(data);if(root.dataset.refv2Signature!==signature){root.dataset.refv2Signature=signature;root.innerHTML=walletMarkup(data)}}
  catch(error){if(root.isConnected)root.innerHTML=`<div class="refv2-error">${esc(error.message||'اطلاعات معرفی دریافت نشد.')}</div>`}
}
async function renderDashboardCode(){
  if(!isDashboard())return;const side=$('.cgr3-dashboard-side')||$('.cgp-hero-side')||$('.cgr3-dashboard-hero')||$('.cgp-hero');if(!side)return;
  try{const data=await loadData();if(!isDashboard()||!side.isConnected)return;const code=data?.caregiver?.referralCode||'—',pending=Number(data?.summary?.awaitingMyConfirmation||0),signature=`${code}:${pending}`;let card=$('#referralDashboardCodeV2');if(!card){card=document.createElement('div');card.id='referralDashboardCodeV2';card.className='refv2-dashboard-code';side.prepend(card)}if(card.dataset.refv2Signature!==signature){card.dataset.refv2Signature=signature;card.innerHTML=`<small>کد معرف من</small><span class="refv2-dashboard-row"><strong>${esc(code)}</strong><button type="button" class="refv2-dashboard-copy" data-refv2-copy="${esc(code)}">کپی</button></span>${pending?`<small class="refv2-dashboard-pending">${fa(pending)} معرفی منتظر تأیید شما</small>`:''}`}}
  catch{}
}
async function decide(id,action){
  if(state.decisionBusy)return;state.decisionBusy=true;
  try{let note='';if(action==='REJECT')note=window.prompt('در صورت تمایل علت رد معرفی را بنویسید:','')||'';await api(`${ENDPOINT}/${encodeURIComponent(id)}`,{method:'PATCH',body:JSON.stringify({action,note})});toast(action==='CONFIRM'?'معرفی تأیید شد':'معرفی رد شد',action==='CONFIRM'?'پرونده برای بررسی مدیر سامانه ارسال شد.':'این معرفی برای مدیر ارسال نمی‌شود.');state.data=null;state.lastLoadedAt=0;schedule(0)}catch(error){toast('ثبت تصمیم انجام نشد',error.message||'دوباره تلاش کنید.')}finally{state.decisionBusy=false}
}
function bindEvents(){
  if(document.documentElement.dataset.refv2Events)return;document.documentElement.dataset.refv2Events='1';
  document.addEventListener('click',event=>{const copy=event.target.closest?.('[data-refv2-copy]');if(copy){event.preventDefault();copyCode(copy.dataset.refv2Copy);return}const confirm=event.target.closest?.('[data-refv2-confirm]');if(confirm){event.preventDefault();decide(confirm.dataset.refv2Confirm,'CONFIRM');return}const reject=event.target.closest?.('[data-refv2-reject]');if(reject){event.preventDefault();decide(reject.dataset.refv2Reject,'REJECT')}});
}
function enforce(){
  addStyles();enhanceSignup();const current=surface();
  if(current==='dashboard'){$('#caregiverReferralRewardsV2')?.remove();renderDashboardCode();return}
  if(current==='wallet'){$('#referralDashboardCodeV2')?.remove();renderWallet();return}
  $('#caregiverReferralRewardsV2')?.remove();$('#referralDashboardCodeV2')?.remove();
}
function schedule(delay=120){clearTimeout(state.timer);state.timer=setTimeout(enforce,delay)}
...
function observe(){
  bindEvents();addStyles();enhanceSignup();schedule(0);
  ...
  window.addEventListener('salamat-access-changed',()=>schedule(0));window.addEventListener('salamat-panel-route-ready',()=>schedule(0));window.addEventListener('popstate',()=>schedule());
  document.addEventListener('visibilitychange',()=>{if(!document.hidden&&surface()!=='other'&&Date.now()-state.lastLoadedAt>30000)schedule()});
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',observe,{once:true});else observe();
window.SalamatReferralRewardsExperienceV2={version:VERSION,refresh:async()=>{state.data=null;state.lastLoadedAt=0;schedule(0)}};
})();