...
async function refresh(){try{const result=await api('/api/notifications?limit=30');cached={items:result.data||[],unread:Number(result.unread||0)};updateBadge();if(!ensurePanel().classList.contains('hidden'))renderPanel()}catch{cached={items:[],unread:0};updateBadge()}}
...
function boot(){document.addEventListener('click',capture,true);document.addEventListener('click',event=>{if(!event.target.closest('#serverNotificationPanel')&&!event.target.closest('.round-btn.notification'))close()});window.addEventListener('salamat-identity-changed',()=>setTimeout(refresh,120));window.addEventListener('salamat-access-changed',()=>setTimeout(refresh,120));void refresh();timer=setInterval(refresh,60000)}
