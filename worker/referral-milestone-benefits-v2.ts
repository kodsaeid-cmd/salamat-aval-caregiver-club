...
const titleFor=(key:string)=>key===NETWORK_KEY?"وام معرفی ۱۰ ثبت‌نام":"وام معرفی با ۷ ورود به قرارداد";
...
async function ensureCohort(env:Env,caregiverId:string,registered?:JsonRecord[]){
 const existing=await loadCohort(env,caregiverId);if(existing)return existing;
 const rows=registered||await registeredReferralRows(env,caregiverId);if(rows.length<NETWORK_TARGET)return null;
 const firstTen=rows.slice(0,NETWORK_TARGET),id=randomId("rfco_"),ts=nowIso(),achievedAt=String(firstTen[NETWORK_TARGET-1]?.referralCreatedAt||ts);
...
async function cohortMembers(env:Env,cohort:any){
 ...
 const rows=await env.DB.prepare(`SELECT r.id,r.referred_caregiver_id AS referredCaregiverId,r.contract_reward_transaction_id AS contractRewardTransactionId,
   c.full_name AS referredName,c.membership_code AS referredMembershipCode,
   CASE WHEN r.contract_reward_transaction_id IS NOT NULL
     OR EXISTS(SELECT 1 FROM caregiver_job_contracts jc WHERE jc.caregiver_id=r.referred_caregiver_id LIMIT 1)
     OR EXISTS(SELECT 1 FROM care_job_applications ap WHERE ap.caregiver_id=r.referred_caregiver_id AND ap.status='IN_CONTRACT' LIMIT 1)
   THEN 1 ELSE 0 END AS contracted
...
export async function buildReferralMilestoneBenefitsSummaryV2(env:Env,caregiverId:string){
 const registered=await registeredReferralRows(env,caregiverId),cohort=await ensureCohort(env,caregiverId,registered),members=cohort?await cohortMembers(env,cohort):registered.slice(0,NETWORK_TARGET),contracted=contractedMembers(members),allContracted=contractedMembers(registered),requests=await existingRequests(env,caregiverId);
 const requestByKey=new Map(requests.map((x:any)=>[String(x.milestoneKey),x]));
 const networkCurrent=cohort?NETWORK_TARGET:Math.min(NETWORK_TARGET,registered.length),contractCurrent=Math.min(CONTRACT_TARGET,contracted.count);
 return {
  policyVersion:"REFERRAL-MILESTONES-2026-08-V4",
  totals:{registered:registered.length,contracted:allContracted.count},
  ...
  network10:{key:NETWORK_KEY,title:titleFor(NETWORK_KEY),amountToman:NETWORK_AMOUNT_TOMAN,target:NETWORK_TARGET,current:networkCurrent,totalQualified:registered.length,remaining:Math.max(0,NETWORK_TARGET-networkCurrent),eligible:Boolean(cohort),request:requestByKey.get(NETWORK_KEY)||null},
...
async function staffAction(request:Request,env:Env,actor:AuthUser,id:string){
 ...
 const latest=await buildReferralMilestoneBenefitsSummaryV2(env,String(current.caregiverId)),tier=String(current.milestoneKey)===NETWORK_KEY?latest.network10:latest.contract7;if(!tier.eligible)return fail("شرایط این پله در زمان تأیید کامل نیست.",409,"referral_milestone_revalidation_failed");
