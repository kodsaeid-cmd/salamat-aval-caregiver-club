...
async function resolveLoginIdentifier(env: Env, rawIdentifier: string): Promise<LoginResolution> {
  ...
  if (normalizedMobile && /^09\d{9}$/.test(normalizedMobile)) {
    ...
    if (account) return {
      identifier: account.username || account.mobile || normalizedMobile,
      caregiver: String(account.role || "").toUpperCase() === "CAREGIVER",
    };
  }
...
export async function normalizeLoginInputV1(request: Request, env: Env): Promise<Request> {
  ...
  const rawPassword = str(body.password);
  const normalizedPassword = normalizeUnicodeDigits(rawPassword);
  if ((resolved.caregiver || /^09\d{9}$/.test(normalizeMobile(normalizeUnicodeDigits(rawIdentifier)) || "")) && /^\d{10}$/.test(normalizedPassword)) {
