...
async function unifiedUsers(request: Request, env: Env, actor: AuthUser) {
  ...
  const createdFrom = str(url.searchParams.get("createdFrom"));
  const createdTo = str(url.searchParams.get("createdTo"));
  ...
  if (statusFilter) { filters.push("upper(COALESCE(status,''))=?"); args.push(statusFilter); }
  if (roleFilter) { filters.push("upper(COALESCE(role,''))=?"); args.push(roleFilter); }
  if (registrationFilter) { filters.push("upper(COALESCE(recruitmentStage,''))=?"); args.push(registrationFilter); }
  if (createdFrom) { filters.push("createdAt>=?"); args.push(createdFrom); }
  if (createdTo) { filters.push("createdAt<?"); args.push(createdTo); }
  ...
  const cte = `WITH directory AS (
    SELECT u.id AS id,u.caregiver_id AS caregiverId,u.full_name AS fullName,
      ...
      COALESCE(c.membership_code,'') AS membershipCode,COALESCE(c.national_id,'') AS nationalId,
      COALESCE(c.recruitment_stage,'') AS recruitmentStage,0 AS profileOnly
    ...
    SELECT '${PROFILE_PREFIX}'||c.id AS id,c.id AS caregiverId,c.full_name AS fullName,c.mobile AS mobile,NULL AS username,
      ...
      COALESCE(c.membership_code,'') AS membershipCode,COALESCE(c.national_id,'') AS nationalId,
      COALESCE(c.recruitment_stage,'') AS recruitmentStage,1 AS profileOnly
    FROM caregivers c
  ...
  return json({ data: rows, pagination: { page, pageSize: PAGE_SIZE, total, totalPages, hasNext: page < totalPages, hasPrevious: page > 1 }, query: q, filters: { status: statusFilter, role: roleFilter, registration: registrationFilter, createdFrom, createdTo } });
}
