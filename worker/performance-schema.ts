...
export async function ensurePerformanceSchema(env: Env) {
  if (!ready) {
    const statements = [
      ...
      "CREATE INDEX IF NOT EXISTS idx_users_upper_status ON users(upper(status))",
      "CREATE INDEX IF NOT EXISTS idx_users_upper_role_status_created ON users(upper(role),upper(status),created_at DESC)",
      "CREATE INDEX IF NOT EXISTS idx_users_caregiver_upper_role_status ON users(caregiver_id,upper(role),upper(status))",
