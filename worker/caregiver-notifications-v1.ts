...
function humanStatus(value:unknown){const s=String(value||"").toUpperCase();return ({FINAL:"نهایی",DRAFT:"در حال ارزیابی",APPROVED:"تأییدشده",UNDER_REVIEW:"در حال بررسی",REQUESTED:"در انتظار بررسی",REJECTED:"ردشده",CANCELLED:"لغوشده",PAID:"پرداخت‌شده"} as Record<string,string>)[s]||str(value)}
function creditTitle(value:unknown){const s=String(value||"").toUpperCase();return ({REQUESTED:"درخواست تسهیلات شما ثبت شد",UNDER_REVIEW:"درخواست تسهیلات شما در حال بررسی است",APPROVED:"تسهیلات شما تأیید شد",REJECTED:"درخواست تسهیلات شما رد شد",CANCELLED:"درخواست تسهیلات شما لغو شد"} as Record<string,string>)[s]||"وضعیت درخواست تسهیلات تغییر کرد"}
...
async function buildItems(env:Env,caregiverId:string):Promise<Item[]>{
 ...
 for(const x of credits){items.push({id:`credit:${x.id}:${x.eventAt}`,moduleKey:"benefits",kind:"CREDIT",title:creditTitle(x.status),body:`${Number(x.amountToman||0).toLocaleString("fa-IR")} تومان • ${humanStatus(x.status)}${x.decisionNote?` • ${str(x.decisionNote)}`:""}`,createdAt:x.eventAt,route:"benefits",amountToman:Number(x.amountToman||0),status:x.status})}
