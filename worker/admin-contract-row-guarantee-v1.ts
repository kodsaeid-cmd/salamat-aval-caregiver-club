import {ensureContractLifecycleV3,reconcileContractCaseByApplication} from "./contract-lifecycle-v3";
...
/**
 * V3 contract cases are keyed by canonical caregiver_job_contract.id, not by job ad.
 * This preserves every historical service period when the same advertisement is reopened.
 */
export async function ensureAdminContractRowForApplication(env:Env,applicationId:string){
 await ensureContractProgressSchema(env);await ensureContractLifecycleV3(env);
 try{const id=await reconcileContractCaseByApplication(env,applicationId);if(id){const ok=await env.DB.prepare("SELECT id FROM contract_cases_v3 WHERE id=? LIMIT 1").bind(id).first<any>();if(ok?.id)return String(ok.id)}}catch(error){console.error("admin_contract_primary_reconcile_failed",{applicationId,error:error instanceof Error?error.message:String(error)})}
 ...
 let existing=await env.DB.prepare("SELECT id FROM contract_cases_v3 WHERE job_contract_id=? LIMIT 1").bind(src.jobContractId).first<any>();
 try{
  if(existing?.id){
   await env.DB.prepare(`UPDATE contract_cases_v3 SET source_application_id=?,contract_title=?,primary_caregiver_id=?,caregiver_salary_rial=?,duration_days=?,starts_at=?,ends_at=?,status=?,renewal_state=?,updated_at=? WHERE id=?`).bind(src.applicationId,title,src.caregiverId,int(src.caregiverSalaryRial),Math.max(1,int(src.durationDays,1)),src.startedAt,src.endsAt,src.jobStatus,state,ts,existing.id).run();
  }else{
   const caseId=randomId("ccv3_");
  ...
  if(providerStatus!=="ACTIVE")await env.DB.prepare("UPDATE contract_service_providers_v3 SET status=?,ended_at=COALESCE(ended_at,?),updated_at=? WHERE contract_case_id=? AND caregiver_id=? AND status='ACTIVE'").bind(providerStatus,endedAt,ts,existing.id,src.caregiverId).run();
  await env.DB.prepare(`INSERT OR IGNORE INTO contract_service_providers_v3(id,contract_case_id,caregiver_id,source_application_id,started_at,ended_at,status,rank_code_snapshot,stars_snapshot,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).bind(randomId("cspv3_"),existing.id,src.caregiverId,src.applicationId,src.startedAt,endedAt,providerStatus,rank.code,rank.stars,ts,ts).run();
 ...
 const verified=await env.DB.prepare("SELECT id FROM contract_cases_v3 WHERE job_contract_id=? LIMIT 1").bind(src.jobContractId).first<any>();return verified?.id?String(verified.id):null;
}

export async function reconcileMissingAdminContractRows(env:Env){
 await ensureContractProgressSchema(env);await ensureContractLifecycleV3(env);
 const rows=await env.DB.prepare(`SELECT jc.application_id AS applicationId FROM caregiver_job_contracts jc LEFT JOIN contract_cases_v3 c ON c.job_contract_id=jc.id WHERE c.id IS NULL ORDER BY jc.created_at ASC LIMIT 1000`).all<{applicationId:string}>();
