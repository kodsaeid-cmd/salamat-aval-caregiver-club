...
type PermissionRow = {moduleKey:string;canView:number|null;canCreate:number|null;canUpdate:number|null;canDelete:number|null};
type PermissionPayload = {moduleKey:string;canView:boolean|null;canCreate:boolean|null;canUpdate:boolean|null;canDelete:boolean|null};
...
export async function individualEffectivePermissions(env:Env,user:AuthUser){
  await ensureAccessControlSchema(env);const role=normalizeRole(user.role),root=isProtectedRootAccount(user),legacy=legacyPermissions(user);
  const [rolePermissions,userPermissions]=await Promise.all([roleRows(env,role),userRows(env,user.id)]);const roleMap=new Map(rolePermissions.map(row=>[row.moduleKey,row])),userMap=new Map(userPermissions.map(row=>[row.moduleKey,row]));
  return MODULE_DEFINITIONS.map(module=>{const roleRow=roleMap.get(module.key),userRow=userMap.get(module.key);const actions=Object.fromEntries(ACTIONS.map(action=>{if(root)return[action,true];const column=ACTION_COLUMN[action],explicitUserValue=userRow?.[column],roleTemplateValue=roleRow?.[column];if(explicitUserValue!==undefined&&explicitUserValue!==null)return[action,Boolean(explicitUserValue)];if(roleTemplateValue!==undefined&&roleTemplateValue!==null)return[action,Boolean(roleTemplateValue)];const fallback=legacy.includes("*")||legacy.includes(module.key)||legacy.includes(`${module.key}.${action}`);return[action,fallback]})) as Record<AccessAction,boolean>;return{...module,actions}});
}
export async function individualCanAccess(env:Env,user:AuthUser|null,moduleKey:string,action:AccessAction="view"){if(!user)return false;const module=(await individualEffectivePermissions(env,user)).find(item=>item.key===moduleKey);return Boolean(module?.actions[action])}
export async function individualRequireAccess(env:Env,user:AuthUser|null,moduleKey:string,action:AccessAction="view"){return await individualCanAccess(env,user,moduleKey,action)?null:fail("برای این عملیات دسترسی ندارید.",403,"forbidden")}
export async function individualAccessMe(env:Env,user:AuthUser){const modules=await individualEffectivePermissions(env,user);const role=ROLE_DEFINITIONS.find(item=>item.key===normalizeRole(user.role));return json({data:{user:{id:user.id,caregiverId:user.caregiverId,fullName:user.fullName,mobile:user.mobile,username:user.username,role:normalizeRole(user.role),roleLabel:role?.label||user.role,status:user.status,protectedRoot:isProtectedRootAccount(user)},panel:role?.panel||"STAFF",modules:modules.filter(module=>module.actions.view),allModules:modules,permissionPolicy:"USER_OVERRIDES_ROLE_TEMPLATE"}})}

function permissionPayload(value:unknown):PermissionPayload|null{if(!value||typeof value!=="object"||Array.isArray(value))return null;const row=value as Record<string,unknown>,moduleKey=str(row.moduleKey||row.key);if(!MODULE_DEFINITIONS.some(module=>module.key===moduleKey))return null;const valueOrNull=(primary:unknown,alternate:unknown)=>primary===undefined&&alternate===undefined?null:Boolean(primary??alternate);return{moduleKey,canView:valueOrNull(row.view,row.canView),canCreate:valueOrNull(row.create,row.canCreate),canUpdate:valueOrNull(row.update,row.canUpdate),canDelete:valueOrNull(row.delete,row.canDelete)}}
async function accountById(env:Env,userId:string){return env.DB.prepare(`SELECT id,caregiver_id AS caregiverId,full_name AS fullName,mobile,username,role,status,permissions_json AS permissionsJson FROM users WHERE id=? LIMIT 1`).bind(userId).first<AuthUser>()}
...
export async function individualUpdateUserPermissions(request:Request,env:Env,actor:AuthUser,userId:string){
  if(normalizeRole(actor.role)!=="ADMIN")return fail("تعیین نقش و دسترسی ماژول‌ها فقط در اختیار مدیر سامانه است.",403,"admin_only");
  if(actor.id===userId)return fail("سطح دسترسی حساب جاری از همین نشست قابل تغییر نیست.",409,"cannot_edit_current_account");
