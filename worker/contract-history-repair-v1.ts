...
async function lifecycleSource(env:Env,applicationId:string){
 return env.DB.prepare(`SELECT ap.id AS applicationId,ap.caregiver_id AS caregiverId,ap.ad_id AS adId,
  COALESCE(ap.lifecycle_status,ap.status) AS lifecycleStatus,ap.applied_at AS appliedAt,ap.updated_at AS updatedAt,
...
/** Materialize an application-only IN_CONTRACT row before any exit/update can erase its history. */
export async function ensureActiveContractForApplication(env:Env,applicationId:string,startedByUserId?:string|null){
 ...
 const src=await lifecycleSource(env,applicationId);if(!src||String(src.lifecycleStatus||"").toUpperCase()!=="IN_CONTRACT")return null;
 const startCandidate=src.legacyAwardedAt||src.updatedAt||src.appliedAt||nowIso(),parsed=Date.parse(String(startCandidate||"")),startedAt=Number.isFinite(parsed)?new Date(parsed).toISOString():nowIso();
 const durationDays=Math.max(1,Math.trunc(Number(src.durationDays||1))),legacyAwarded=src.legacyPoints!=null;
 const allocatedUnits=units(src.contractPoints),legacyUnits=legacyAwarded?units(src.legacyPoints):0,totalUnits=Math.max(allocatedUnits,legacyUnits),id=randomId("jct_"),ts=nowIso();
 try{
  await env.DB.prepare(`INSERT INTO caregiver_job_contracts(id,caregiver_id,ad_id,application_id,started_at,scheduled_end_at,duration_days,total_points_units,earned_points_units,last_reconciled_day,status,points_model,started_by_user_id,welcome_seen_at,created_at,updated_at)
   VALUES(?,?,?,?,?,?,?,?,?,0,'ACTIVE',?,?,NULL,?,?)`).bind(id,src.caregiverId,src.adId,src.applicationId,startedAt,plusDays(startedAt,durationDays),durationDays,totalUnits,legacyUnits,legacyAwarded?"LEGACY_PREPAID":"DAILY_V1",startedByUserId||null,ts,ts).run();
 }catch(error:any){
  const raced=await byApplication(env,applicationId);if(raced)return raced;
  console.error("active_contract_row_backfill_failed",{applicationId,adId:src.adId,error:error instanceof Error?error.message:String(error)});return null;
 }
...
export async function ensureHistoricalContractForApplication(env:Env,applicationId:string,endedByUserId?:string|null){
 await ensureJobApplicationLifecycleSchema(env);await ensureContractProgressSchema(env);
 const existing=await byApplication(env,applicationId);if(existing)return existing;
 const src=await lifecycleSource(env,applicationId);if(!src)return null;
 const lifecycle=String(src.lifecycleStatus||"").toUpperCase();if(!["WITHDRAWN","COMPLETED"].includes(lifecycle))return null;
 const endedRaw=Date.parse(String(src.updatedAt||"")),endedAt=Number.isFinite(endedRaw)?new Date(endedRaw).toISOString():nowIso();
...
export async function reconcileCanonicalOpenContractRows(env:Env){
 await ensureJobApplicationLifecycleSchema(env);await ensureContractProgressSchema(env);
 const rows=await env.DB.prepare(`SELECT ap.id AS applicationId FROM care_job_applications ap
  ...
  WHERE COALESCE(ap.lifecycle_status,ap.status)='IN_CONTRACT' AND jc.id IS NULL
  ORDER BY ap.updated_at ASC LIMIT 1000`).all<{applicationId:string}>();
 let discovered=0,backfilled=0,failed=0;
 for(const row of rows.results||[]){discovered++;try{if(await ensureActiveContractForApplication(env,row.applicationId))backfilled++;else failed++}catch(error){failed++;console.error("canonical_open_contract_batch_backfill_failed",{applicationId:row.applicationId,error:error instanceof Error?error.message:String(error)})}}
