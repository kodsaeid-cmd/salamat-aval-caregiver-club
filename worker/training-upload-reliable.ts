...
export async function uploadTrainingCourse(request: Request,env: Env,actor: AuthUser) {
  ...
  if (!uploadedResponse.ok) return json({error:uploadedPayload.error||"training_file_upload_failed",message:uploadedPayload.message||"بارگذاری فایل آموزشی انجام نشد.",stage:"file_upload"},uploadedResponse.status);

  const fileId = str(uploadedPayload?.data?.id);
  if (!fileId) return fail("ثبت فایل آموزشی کامل نشد.", 500, "uploaded_file_id_missing");
  const contentUrl = `/api/files/${encodeURIComponent(fileId)}/download?inline=1`;
  const createRequest = new Request(request.url, {method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
    title,
  ...
  if (!courseResponse.ok) {
    ...
    return json({error:coursePayload.error||"training_course_save_failed",message:coursePayload.message||"ثبت مشخصات آموزش انجام نشد.",stage:"course_save"},courseResponse.status);
  }
  return json({data:{course:coursePayload.data,file:uploadedPayload.data}},201);
}
