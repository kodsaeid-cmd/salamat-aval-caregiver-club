...
async function sanitizeLoginSample(request: Request, response: Response) {if (request.method.toUpperCase() !== "GET" || !response.ok) return response;const contentType = response.headers.get("content-type") || "";if (!contentType.includes("text/html")) return response;let html = await response.text();if (!html.includes(LOGIN_SAMPLE_MOBILE)) return new Response(html, response);html = html.replaceAll(`value="${LOGIN_SAMPLE_MOBILE}"`, "value=\"\"").replaceAll(`value='${LOGIN_SAMPLE_MOBILE}'`, "value=''").replaceAll(`placeholder="${LOGIN_SAMPLE_MOBILE}"`, "placeholder=\"09xxxxxxxxx\"").replaceAll(`placeholder='${LOGIN_SAMPLE_MOBILE}'`, "placeholder='09xxxxxxxxx'").replaceAll(LOGIN_SAMPLE_MOBILE, "");const headers = new Headers(response.headers);headers.delete("content-length");headers.set("cache-control", "private, no-store, max-age=0");headers.set("x-salamat-login-sample", "removed");return new Response(html, { status: response.status, statusText: response.statusText, headers });}
...
function shouldCheckDesktopSession(request: Request, url: URL) {if (isMobileClient(request) || desktopClassicRequested(url)) return false;if (!["GET", "HEAD"].includes(request.method.toUpperCase())) return false;return ["/", "/panel", "/panel/"].includes(url.pathname);}
...
export default {
  ...
  async scheduled(controller: WorkerScheduledController, env: any, ctx: WorkerLifecycleContext) {ctx.waitUntil(reconcileAllActiveContracts(env));if (typeof app.scheduled === "function") return app.scheduled(controller, env, ctx);}
};
