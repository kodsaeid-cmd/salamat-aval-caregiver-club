...
const DESKTOP_REACT_VERSION = "1.5.10";
...
async function sessionRole(request: Request, env: any, ctx: WorkerLifecycleContext) {
  try {
    const authUrl = new URL(request.url);authUrl.pathname = "/api/auth/me";authUrl.search = "";
    const authRequest = new Request(authUrl.toString(), { method: "GET", headers: request.headers });
    const response = await app.fetch(authRequest, env, ctx);if (!response.ok) return "";
    const payload: any = await response.json().catch(() => null);return String(payload?.data?.role || "").toUpperCase();
  } catch {return "";}
}
...
function desktopHeaders(response: Response, documentResponse = false) {
  const headers = new Headers(response.headers);headers.delete("content-length");headers.delete("pragma");headers.delete("expires");
  headers.set("cache-control", documentResponse ? "private, no-store, max-age=0" : "public, max-age=0, must-revalidate");
  headers.set("x-salamat-desktop-owner", `react-${DESKTOP_REACT_VERSION}`);headers.set("x-salamat-desktop-react", DESKTOP_REACT_VERSION);headers.set("x-salamat-desktop-layer-count", "1");
...
async function sanitizeLoginSample(request: Request, response: Response) {
  ...
  let html = await response.text();if (!html.includes(LOGIN_SAMPLE_MOBILE)) return new Response(html, response);
  html = html.replaceAll(`value="${LOGIN_SAMPLE_MOBILE}"`, "value=\"\"").replaceAll(`value='${LOGIN_SAMPLE_MOBILE}'`, "value=''").replaceAll(`placeholder="${LOGIN_SAMPLE_MOBILE}"`, "placeholder=\"09xxxxxxxxx\"").replaceAll(`placeholder='${LOGIN_SAMPLE_MOBILE}'`, "placeholder='09xxxxxxxxx'").replaceAll(LOGIN_SAMPLE_MOBILE, "");
  const headers = new Headers(response.headers);headers.delete("content-length");headers.set("cache-control", "private, no-store, max-age=0");headers.set("x-salamat-login-sample", "removed");
  return new Response(html, { status: response.status, statusText: response.statusText, headers });
}

async function serveDesktopReact(request: Request, env: any) {
  const url = new URL(request.url);if (url.pathname === "/app") {url.pathname = "/app/";return Response.redirect(url.toString(), 302);}
  const isAsset = /\.(?:js|css|webmanifest|svg|png|webp|jpg|jpeg|ico)$/i.test(url.pathname);if (isAsset) return desktopHeaders(await env.ASSETS.fetch(request), false);
  const assetUrl = new URL(request.url);assetUrl.pathname = DESKTOP_REACT_INDEX;assetUrl.search = "";
  const indexRequest = new Request(assetUrl.toString(), { method: request.method, headers: request.headers });return desktopHeaders(await env.ASSETS.fetch(indexRequest), true);
}

function shouldCheckDesktopSession(request: Request, url: URL) {
  if (isMobileClient(request) || desktopClassicRequested(url)) return false;if (!["GET", "HEAD"].includes(request.method.toUpperCase())) return false;return ["/", "/panel", "/panel/"].includes(url.pathname);
}
  ...
  async fetch(request: Request, env: any, ctx: WorkerLifecycleContext) {
    ...
    if (jobAdsResponse) {
      if (lifecyclePatch && jobAdsResponse.ok && String(lifecycleBody?.status || "").toUpperCase() === "IN_CONTRACT") {
        try {await reconcileContractCaseByApplication(env, decodeURIComponent(lifecyclePatch[2]));}
        catch (error) {console.error("contract_case_immediate_reconcile_failed", {applicationId: decodeURIComponent(lifecyclePatch[2]),adId: decodeURIComponent(lifecyclePatch[1]),error: error instanceof Error ? error.message : String(error)});}
      }
    ...
    if (shouldCheckDesktopSession(request, url)) {
      const role = await sessionRole(request, env, ctx);if (STAFF_ROLES.has(role) || role === "CAREGIVER") {const target = new URL(request.url);target.pathname = role === "CAREGIVER" ? "/mobile/" : "/app/";target.search = "";return Response.redirect(target.toString(), 302);}
    }
    let response = await delegateProtectedApp(request, env, ctx);
    if (lifecyclePatch && response.ok && String(lifecycleBody?.status || "").toUpperCase() === "IN_CONTRACT") ctx.waitUntil(reconcileContractCaseByApplication(env, decodeURIComponent(lifecyclePatch[2])).catch(() => undefined));
    response = await rewriteJobAdsAccessResponse(request, response);response = await rewriteFinancialResponseWithPoints(request, env, response);response = await rewriteSalesSupervisorAccessV1(request,response);return sanitizeLoginSample(request, response);
  },
  async scheduled(controller: WorkerScheduledController, env: any, ctx: WorkerLifecycleContext) {
    ctx.waitUntil(reconcileAllActiveContracts(env));if (typeof app.scheduled === "function") return app.scheduled(controller, env, ctx);
  },
};
