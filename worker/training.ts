...
type AssignmentStatus = "ASSIGNED" | "IN_PROGRESS" | "COMPLETED" | "CANCELLED" | string;
...
interface TrainingAssignment extends Omit<TrainingAssignmentRow, "mandatory"> {
  mandatory: boolean;
  assignedByRoleLabel: string;
}
...
function normalizeAssignment(row: TrainingAssignmentRow): TrainingAssignment {
  return {
    ...row,
  ...
  if (!caregiverId) {
    return fail(
      actor.role.toUpperCase() === "CAREGIVER"
        ? "حساب شما به پرونده مراقب متصل نیست."
        : "شناسه مراقب لازم است.",
      409,
      "caregiver_profile_missing",
    );
  }
...
export async function createCourse(request: Request, env: Env, actor: AuthUser) {
  ...
  try {
    await env.DB.prepare(`INSERT INTO courses(
...
export async function assignCourse(request: Request, env: Env, actor: AuthUser) {
  ...
  const course = await env.DB.prepare("SELECT id,title FROM courses WHERE id=? AND status='ACTIVE' LIMIT 1")
