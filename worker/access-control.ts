...
async function roleRows(env: Env, role: string) {
  const result = await env.DB.prepare(`SELECT
    ...
    FROM role_module_permissions WHERE role=?`).bind(role).all<PermissionRow>();
...
async function userRows(env: Env, userId: string) {
  const result = await env.DB.prepare(`SELECT
    module_key AS moduleKey,can_view AS canView,can_create AS canCreate,
    can_update AS canUpdate,can_delete AS canDelete
    FROM user_module_permissions WHERE user_id=?`).bind(userId).all<PermissionRow>();
  return result.results || [];
}
...
export async function effectivePermissions(env: Env, user: AuthUser) {
  ...
  return MODULE_DEFINITIONS.map((module) => {
    ...
    const actions = Object.fromEntries(ALL_ACTIONS.map((action) => {
      ...
      return [action, role === "ADMIN" || legacyAllowed || Boolean(overrideValue ?? baseValue ?? 0)];
...
export async function requireAccess(
  ...
  return await canAccess(env, user, moduleKey, action)
    ? null
...
export async function accessConfiguration(env: Env, user: AuthUser) {
  if (user.role.toUpperCase() !== "ADMIN") return fail("فقط مدیر سامانه می‌تواند سیاست دسترسی را تغییر دهد.", 403, "forbidden");
  await ensureAccessControlSchema(env);
  const result = await env.DB.prepare(`SELECT role,module_key AS moduleKey,
    can_view AS canView,can_create AS canCreate,can_update AS canUpdate,can_delete AS canDelete
    FROM role_module_permissions ORDER BY role,module_key`).all<Record<string, unknown>>();
  return json({
    data: {
      roles: ROLE_DEFINITIONS,
      modules: MODULE_DEFINITIONS,
      rolePermissions: result.results || [],
    },
  });
}
...
export async function updateRolePermissions(request: Request, env: Env, actor: AuthUser, roleValue: string) {
  if (actor.role.toUpperCase() !== "ADMIN") return fail("فقط مدیر سامانه می‌تواند سیاست نقش‌ها را تغییر دهد.", 403, "forbidden");
  ...
  await ensureAccessControlSchema(env);
  const timestamp = nowIso();
  await env.DB.batch(permissions.map((permission) => env.DB.prepare(`INSERT INTO role_module_permissions(
    role,module_key,can_view,can_create,can_update,can_delete,updated_by_user_id,updated_at
  ) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(role,module_key) DO UPDATE SET
    can_view=excluded.can_view,can_create=excluded.can_create,can_update=excluded.can_update,
    can_delete=excluded.can_delete,updated_by_user_id=excluded.updated_by_user_id,updated_at=excluded.updated_at`)
    .bind(
      role,
  ...
  await audit(request, env, actor, "UPDATE_ROLE_PERMISSIONS", "role", role, permissions);
  return json({ ok: true, role, updatedAt: timestamp });
}

export async function getUserPermissions(env: Env, actor: AuthUser, userId: string) {
  if (actor.role.toUpperCase() !== "ADMIN") return fail("فقط مدیر سامانه می‌تواند دسترسی کاربران را مشاهده کند.", 403, "forbidden");
  await ensureAccessControlSchema(env);
  const user = await env.DB.prepare(`SELECT id,caregiver_id AS caregiverId,full_name AS fullName,mobile,username,
    role,status,permissions_json AS permissionsJson,created_at AS createdAt,updated_at AS updatedAt
    FROM users WHERE id=? LIMIT 1`).bind(userId).first<AuthUser & Record<string, unknown>>();
...
export async function updateUserPermissions(request: Request, env: Env, actor: AuthUser, userId: string) {
  if (actor.role.toUpperCase() !== "ADMIN") return fail("فقط مدیر سامانه می‌تواند دسترسی کاربران را تغییر دهد.", 403, "forbidden");
  ...
  const timestamp = nowIso();
  const statements = [
    env.DB.prepare("UPDATE users SET role=?,updated_at=? WHERE id=?").bind(role, timestamp, userId),
  ...
  await audit(request, env, actor, "UPDATE_USER_PERMISSIONS", "user", userId, { role, permissions });
