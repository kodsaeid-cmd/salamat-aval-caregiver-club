...
const RUNTIME_FILES=['access-control-runtime-v2.js','staff-module-router-v3.js','staff-financial-credits-runtime-v2.js','staff-payroll-runtime-v1.js','staff-support-runtime-v1.js'];
const EXPECTED_MODULES=['staff.dashboard','staff.users','staff.caregivers','staff.contracts','staff.payroll','staff.financial_credits','staff.training','staff.evaluations','staff.support','staff.settings'];
const checks=[];
...
async function fetchRuntime(file){const response=await fetch(`${baseUrl}/${file}?v=${PLATFORM}&asset=${Date.now()}`,{cache:'no-store',headers:{'cache-control':'no-cache'}});const text=await response.text();return{status:response.status,type:response.headers.get('content-type')||'',text}}
async function waitForRelease(){
  const deadline=Date.now()+240_000;let last='no response';
  while(Date.now()<deadline){
    const version=await request(`/api/system/version?router-v5=${Date.now()}`).catch(error=>({response:{status:0},body:null,error}));
    const html=await fetch(`${baseUrl}/?router-v5=${Date.now()}`,{cache:'no-store',headers:{'cache-control':'no-cache'}}).then(async response=>({response,text:await response.text()})).catch(error=>({response:{status:0,headers:new Headers()},text:'',error}));
    const assets=await Promise.all(RUNTIME_FILES.map(async file=>[file,await fetchRuntime(file).catch(error=>({status:0,type:'',text:String(error)}))]));
    const assetMap=Object.fromEntries(assets);const assetsReady=assets.every(([,asset])=>asset.status===200&&/javascript|text\/plain/.test(asset.type)&&!/<html/i.test(asset.text.slice(0,200)));
    const ready=version.response.status===200&&version.body?.caregiverPlatform===PLATFORM&&version.body?.adminRouter===ROUTER&&version.body?.accessControl===ACCESS&&version.body?.frontendContract==='caregiver-platform-v2-router-v5-access-v2'&&html.response.status===200&&html.response.headers.get('x-salamat-caregiver-platform')===PLATFORM&&html.response.headers.get('x-salamat-admin-router')===ROUTER&&html.response.headers.get('x-salamat-access-control')===ACCESS&&html.text.includes(`access-control-runtime-v2.js?v=${PLATFORM}`)&&!/(?:^|\/)access-control-runtime\.js(?:\?|["'])/.test(html.text)&&html.text.includes(`staff-module-router-v3.js?v=${PLATFORM}`)&&assetsReady;
    if(ready)return{version:version.body,html,assetMap};
    last=JSON.stringify({version:version.body,htmlStatus:html.response.status,platform:html.response.headers.get?.('x-salamat-caregiver-platform'),router:html.response.headers.get?.('x-salamat-admin-router'),access:html.response.headers.get?.('x-salamat-access-control'),assets:Object.fromEntries(assets.map(([file,asset])=>[file,{status:asset.status,type:asset.type,preview:asset.text.slice(0,40)}]))});
...
const release=await waitForRelease();passed('release.router-v5-assets');
const login=await request('/api/auth/login',{method:'POST',body:JSON.stringify({identifier:rootUser.username,password})});expect(login.response.status===200,`login returned ${login.response.status}: ${JSON.stringify(login.body)}`);const cookie=sessionCookie(login.response);expect(cookie.startsWith('salamat_session='),'session cookie missing');passed('root.login');
const access=await authed(cookie,'/api/access/me');expect(access?.data?.panel==='STAFF','root did not enter staff panel');expect(access?.data?.moduleContractVersion==='3.0.0','module contract is not 3.0.0');const modules=access.data.modules.map(module=>module.key);expect(JSON.stringify(modules)===JSON.stringify(EXPECTED_MODULES),`visible module order differs: ${JSON.stringify(modules)}`);expect(!modules.includes('staff.reports'),'reports remain visible');passed('root.ten-module-contract');
...
const evidence={caregiverPlatform:PLATFORM,adminRouter:ROUTER,accessControl:ACCESS,visibleModules:EXPECTED_MODULES,runtimeAssets:RUNTIME_FILES,pollingAccessRuntimeRemoved:true,checks,verifiedAt:new Date().toISOString()};fs.mkdirSync('.admin-core-smoke',{recursive:true,mode:0o700});fs.writeFileSync('.admin-core-smoke/result-v2.json',JSON.stringify(evidence,null,2),{mode:0o600});console.log(`Admin API smoke v2 passed with ${checks.length} checks.`);
