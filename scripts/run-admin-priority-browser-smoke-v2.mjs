...
try{
 ...
 expect(errors.length===0,`browser errors: ${errors.join(' | ')}`);fs.writeFileSync(resultPath,JSON.stringify({desktopReact:{labels:desktopLabels,routes:desktopRoutes,modules:11},mobileAdmin,caregiverScorecard,browserErrors:errors,verifiedAt:new Date().toISOString()},null,2),{mode:0o600});console.log('Admin priority browser smoke v2 passed: 11 desktop React modules, mobile admin critical routes and caregiver scorecard tabs are operational.');
