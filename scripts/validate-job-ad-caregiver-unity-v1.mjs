...
const mobileShell=read('mobile-react/admin.tsx');
...
expect(mobileEntry.includes('/api/access/me')&&mobileEntry.includes('panel')&&mobileEntry.includes('STAFF')&&!mobileEntry.includes('STAFF_ROLES'), 'mobile entry is not governed by canonical STAFF panel access');
expect(mobileShell.includes('/api/access/me')&&mobileShell.includes('panel')&&mobileShell.includes('STAFF')&&!mobileShell.includes('STAFF_ROLES'), 'mobile staff shell still depends on a hard-coded role allowlist');
...
console.log(`Unified job-ad caregiver behavior, role presets, STAFF-panel mobile routing, notifications, applicant stars and four-tab records are valid via ${mobileOwner} / ${desktopOwner}.`);
