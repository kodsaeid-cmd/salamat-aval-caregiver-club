...
requireText(shell,"const VERSION='5.0.0'",'mobile shell version must be 5.0.0');
...
requireText(shell,'sourceNav().filter','dashboard grid must filter the real desktop module list');
requireText(shell,"clickSource(modules[index])",'dashboard module tiles must delegate to the original desktop module buttons');
...
requireOrder(shell,["{key:'profile',label:'پروفایل'","{key:'calendar',label:'تقویم'","{key:'home',label:'خانه'","{key:'support',label:'پشتیبانی'","{key:'training',label:'آموزش'"],'bottom navigation must be RTL: profile, calendar, home, support, training');
...
requireText(worker,'const MOBILE_CAREGIVER_SHELL_VERSION = "5.0.0"','worker mobile shell version is missing');
