...
for (const compatibilityMarker of ['class PanelLoginCompatibility','data-salamat-panel-compat','id="roleOptions"','id="methodTabs"','id="sendOtp"','id="loginForm"']) assert.ok(worker.includes(compatibilityMarker), `Missing panel compatibility marker: ${compatibilityMarker}`);
...
for (const marker of ["const PANEL_PATH='/panel'","AUTH_PATHS=new Set(['/api/auth/login','/api/auth/verify-otp'])",'location.replace(PANEL_PATH)','return new Promise(()=>{})']) assert.ok(transition.includes(marker), `Missing login transition marker: ${marker}`);

for (const marker of ["const VERSION='1.3.0'","PANEL_PATH='/panel'",'stabilizeCompatibilitySurface()',"login.style.setProperty('display','none','important')","$('#caregiverSignupLayer')?.remove()",'window.SalamatStaffModuleRouter','router.route(key)',"releasePanel('staff-route-dispatched')"]) assert.ok(panel.includes(marker), `Missing panel document marker: ${marker}`);
...
for (const marker of ["const PANEL_PATH='/panel'","const LOGIN_PATH='/'",'if(payload?.data)location.replace(PANEL_PATH)','location.replace(LOGIN_PATH)','if(!panelRoute){location.replace(PANEL_PATH);return}','if(error.status===401&&panelRoute){location.replace(LOGIN_PATH);return}']) assert.ok(backend.includes(marker), `Missing backend route marker: ${marker}`);
...
for (const obsolete of ['../preview/auth-surface-gate-v1.js','./validate-auth-surface-gate-v1.mjs']) {let exists = true;try { await access(new URL(obsolete, import.meta.url)); } catch { exists = false; }assert.equal(exists, false, `Obsolete same-document surface file still exists: ${obsolete}`)}
