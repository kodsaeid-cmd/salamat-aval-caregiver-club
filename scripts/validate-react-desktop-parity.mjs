...
for(const value of ["STAFF_ROLES=new Set(['ADMIN','RECRUITER','HR','SUPPORT','EVALUATOR','EDUCATION','OPERATIONS','SALES_CONSULTANT'])","reactDesktopTarget(user)","if(role==='CAREGIVER')return '/mobile/'","if(STAFF_ROLES.has(role))return '/app/'","classicRequested()","/api/auth/login","salamat-authenticated","legacyMobileSubmit(event)","pollAuthenticatedSession()","fetch('/api/auth/me'","window.addEventListener('salamat-authenticated',authenticatedEvent)"])has(login,value,`desktop login continuity: ${value}`);
has(loginWorker,'login-identifier-compat.js?v=3.4.0','production login worker must publish sales-consultant-aware login asset');
